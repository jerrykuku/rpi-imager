...
Item {
    ...
    function isCustomizationSubstepConfigured(subIndex) {
        ...
        if (subIndex >= labels.length)
            return false;

        var stepLabel = labels[subIndex];
        if (stepLabel === qsTr("Hostname"))
            return hostnameConfigured;
        if (stepLabel === qsTr("Localisation"))
            return localeConfigured;
        if (stepLabel === qsTr("User"))
            return userConfigured;
        if (stepLabel === qsTr("Wi‑Fi"))
            return wifiConfigured;
        if (stepLabel === qsTr("Remote access"))
            return sshEnabled;
                    ...
                    Repeater {
                        ...
                        Rectangle {
                            ...
                            Column {
                                ...
                                visible: false  // Customisation steps removed from wizard flow
                                ...
                                Repeater {
                                    ...
                                    Rectangle {
                                        ...
                                        property bool isCurrentStep: {
                                            ...
                                            if (subItem.index >= labels.length)
                                                return false;

                                            var currentStepLabel = "";
                                            if (root.currentStep === root.stepHostnameCustomization)
                                                currentStepLabel = qsTr("Hostname");
                                            else if (root.currentStep === root.stepLocaleCustomization)
                                                currentStepLabel = qsTr("Localisation");
                                            else if (root.currentStep === root.stepUserCustomization)
                                                currentStepLabel = qsTr("User");
                                            else if (root.currentStep === root.stepWifiCustomization)
                                        ...
                                        MouseArea {
                                            ...
                                            onClicked: {
                                                ...
                                                if (stepLabel === qsTr("Hostname"))
                                                    target = root.stepHostnameCustomization;
                                                else if (stepLabel === qsTr("Localisation"))
                                                    target = root.stepLocaleCustomization;
                                                else if (stepLabel === qsTr("User"))
                                                    target = root.stepUserCustomization;
                                                else if (stepLabel === qsTr("Wi‑Fi"))
                                        ...
                                        RowLayout {
                                            ...
                                            MarqueeText {
                                                ...
                                                color: (!root.customizationSupported || !subItem.isClickable) ? Style.formLabelDisabledColor : Style.sidebarTextOnInactiveColor
                                            }
                                        }
                                    }
                                }
    ...
    function nextStep() {
        if (root.currentStep < root.totalSteps - 1) {
            ...
            // Skip customisation steps entirely: go directly from storage to writing
            if (root.currentStep === stepStorageSelection) {
                nextIndex = stepWriting;
                writeAnotherMode = false;  // Reset the flag if set
            }
    ...
    function previousStep() {
        if (root.currentStep > 0) {
            ...
            // From Writing step: jump straight back to Storage Selection (customisation skipped)
    ...
    function getStepComponent(stepIndex) {
        ...
        case stepDeviceSelection:
            return deviceSelectionStep;
        case stepOSSelection:
            return osSelectionStep;
        case stepStorageSelection:
            return storageSelectionStep;
        case stepHostnameCustomization:
            return hostnameCustomizationStep;
        case stepLocaleCustomization:
            return localeCustomizationStep;
        case stepUserCustomization:
            return userCustomizationStep;
        case stepWifiCustomization:
            return wifiCustomizationStep;
        case stepRemoteAccess:
    ...
    Component {
        ...
        OSSelectionStep {
            ...
            onUpdatePopupRequested: function (url, version) {
                root.updatePopupRequested(url, version);
            }
        }
    }
